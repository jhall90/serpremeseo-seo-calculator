...
class SeoCalculator extends HTMLElement {
    ...
    constructor() {
        super(),
            ...
            (this._chart = null),
            (this._themeAttr = this.getAttribute("theme") || "system");
    }
    get template() {
        return `
<style>
...
<div class="calculator">
    ...
    connectedCallback() {
        this._cacheDom(),
            this._readPageCssVars(),
            this._applyAttributeOverrides(),
            this._initThemeListener(),
            this._updateThemeClass(),
            this._updateResponsiveClasses(),
    ...
    _cacheDom() {
        ...
        (this.$pageUpdates = t.getElementById("pageUpdates")),
            (this.$blogs = t.getElementById("blogs")),
            (this.$videos = t.getElementById("videos")),
            (this.$backlinks = t.getElementById("backlinks")),
            (this.$techSeo = t.getElementById("techSeo")),
            (this.$reporting = t.getElementById("reporting")),
            (this.$contractLength = t.getElementById("contractLength")),
            (this.$monthlyCost = t.getElementById("monthlyCost")),
            (this.$oneTimeCost = t.getElementById("oneTimeCost")),
            (this.$pagesLabel = t.getElementById("pagesLabel")),
            (this.$blogsLabel = t.getElementById("blogsLabel")),
            (this.$videosLabel = t.getElementById("videosLabel")),
            (this.$backlinksLabel = t.getElementById("backlinksLabel")),
            (this.$techLabel = t.getElementById("techLabel")),
            (this.$reportingLabel = t.getElementById("reportingLabel")),
            (this.$termLabel = t.getElementById("termLabel")),
            (this.$chartCanvas = t.getElementById("paymentChart")),
            (this.$themeToggle = t.querySelector(".theme-toggle"));
    }
    _readPageCssVars() {
        const t = [
                "--brand-top",
        ...
        t.forEach((o) => {
            const a = e.getPropertyValue(o).trim();
            a && this.style.setProperty(o, a);
        });
    }
    _applyAttributeOverrides() {
        [...this.attributes].forEach((t) => {
            const e = "--" + t.name.replace(/-(.)/g, (o, a) => a.toLowerCase());
    ...
    _initThemeListener() {
        this.$themeToggle.addEventListener("click", () => {
            const t = this.getAttribute("theme") || "system",
                e = t === "light" ? "dark" : t === "dark" ? "system" : "light";
    ...
    _updateThemeClass() {
        const t = this.getAttribute("theme") || "system",
            ...
            o = document.body.classList.contains("dark-mode");
        let a = !1;
        t === "dark" ? (a = !0) : t === "light" ? (a = !1) : (a = o || e),
            a ? this.classList.add("dark") : this.classList.remove("dark");
    }
    _loadChartJs() {
        return window.Chart
            ? Promise.resolve()
    ...
    _centerTextPlugin = {
        ...
        afterDraw: (t) => {
            const {
                    ctx: e,
                    chartArea: { width: o, height: a },
                } = t,
                i = t.$centerText;
            if (!i || i.length === 0) return;
            e.save(), (e.textAlign = "center"), (e.textBaseline = "middle");
            const r = o / 2 + t.chartArea.left,
                l = a / 2 + t.chartArea.top,
                n = 22;
            i.forEach((s, h) => {
                (e.fillStyle = s.color || "#ffffff"),
                    (e.font = s.font || "600 20px sans-serif");
                const p = (h - (i.length - 1) / 2) * n;
                e.fillText(s.text, r, l + p);
            }),
                e.restore();
        },
    };
    _initChart() {
        const t = this.$chartCanvas.getContext("2d");
        (this._chart = new Chart(t, {
            ...
            options: {
                ...
                onHover: (e, o) => {
                    if (o.length > 0) {
                        const a = o[0].index;
                        this._updateCenterForSlice(a);
                    } else this._updateCenterDefault();
                },
            },
        })),
    ...
    _updateCenterDefault() {
        const t = this._currentTotal || 0;
        (this._chart.$centerText = [
            { text: `$${t.toLocaleString()}`, font: "700 26px sans-serif" },
            {
                text: "/mo",
    ...
    _updateCenterForSlice(t) {
        const e = this._chart.data.datasets[0].data,
            o = e.reduce((l, n) => l + n, 0),
            a = e[t];
        if (o === 0) return;
        const i = Math.round((a / o) * 100),
            r = this._chart.data.labels[t];
        (this._chart.$centerText = [
            { text: `${i}%`, font: "700 26px sans-serif" },
            {
                text: r,
    ...
    _updateChartColors() {
        this._chart &&
            ((this._chart.data.datasets[0].backgroundColor = [
                this._css("--chart-onpage"),
                this._css("--chart-tech"),
                this._css("--chart-offpage"),
                this._css("--chart-reporting"),
    ...
    _attachEvents() {
        [
            ...
            t.addEventListener("input", () => this._updateCalculator())
        );
    }
    _updateCalculator() {
        const r = { 1: 0, 3: 0.05, 6: 0.1, 12: 0.15 },
            l = (x) => (x >= 100 ? 0.1 : x >= 40 ? 0.05 : 0),
            n = Number(this.$pageUpdates.value) || 0,
            s = Number(this.$blogs.value),
            h = Number(this.$videos.value),
            p = Number(this.$backlinks.value),
            g = Number(this.$contractLength.value);
        let u = n * 425;
        u *= 1 - l(n);
        const y = this.$reporting.value === "one" ? 1e3 : 0,
            k = Math.round(u + y);
        this.$oneTimeCost.textContent = `$${k.toLocaleString()}`;
        const C = s * 375,
            $ = this.$techSeo.value === "yes" ? 600 : 0,
            _ = h * 675 + p * 625;
        let c = 0;
        this.$reporting.value === "monthly" && (c = 600),
            this.$reporting.value === "quarterly" && (c = 333);
        const d = r[g] || 0,
            b = Math.ceil(C * (1 - d)),
            m = Math.ceil($ * (1 - d)),
            f = Math.ceil(_ * (1 - d)),
            v = c > 0 ? Math.ceil(c * (1 - d)) : 0,
            w = b + m + f + v;
        (this.$monthlyCost.textContent = `$${w.toLocaleString()}`),
            (this.$pagesLabel.textContent = n > 0 ? `${n} pages` : "None"),
            (this.$blogsLabel.textContent = s > 0 ? `${s} / month` : "None"),
            (this.$videosLabel.textContent = h > 0 ? `${h} / month` : "None"),
            (this.$backlinksLabel.textContent = p > 0 ? `${p} links` : "None"),
            (this.$techLabel.textContent =
                this.$techSeo.value === "yes" ? "$600 / month" : "None"),
            (this.$reportingLabel.textContent =
                this.$reporting.value === "one"
                    ? "One-Time"
                    : this.$reporting.value === "quarterly"
                    ? "Quarterly ($333 / mo)"
                    : this.$reporting.value === "monthly"
                    ? "Monthly ($600 / mo)"
                    : "None"),
            (this.$termLabel.textContent =
                g === 1 ? "One Month" : `${g} Months`),
            this._updateChartData(b, m, f, v);
    }
    _updateChartData(t, e, o, a) {
        const i = [t, e, o, a];
        (this._chart.data.datasets[0].data = i),
            (this._currentTotal = t + e + o + a),
            this._chart.update(),
            this._updateCenterDefault();
    }
    _updateResponsiveClasses() {
        const t = window.innerWidth,
            o = window.innerHeight > t,
            a = t <= 550,
            i = t > 550 && t <= 1024;
        if ((this.classList.remove("mobile", "tablet"), a)) {
...
customElements.define("seo-calculator", SeoCalculator);
