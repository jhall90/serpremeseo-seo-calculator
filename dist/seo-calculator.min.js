const L = `<div class="calculator">\r
...
class z extends HTMLElement {
  ...
  _applyOverrides() {
    const t = this.getAttribute("branding"), e = ["brand", "minimal", "off"].includes(t) ? t : "brand", n = O[e] || [];
    ...
    const l = this.getAttribute("style");
    l && l.split(";").forEach((i) => {
      const r = i.indexOf(":");
      if (r === -1) return;
  ...
  _centerTextPlugin = {
    ...
    afterDraw: (t) => {
      ...
      if (!a?.length) return;
      const l = n.left + n.width / 2, i = n.top + n.height / 2;
      e.save(), e.textAlign = "center", e.textBaseline = "middle", a.forEach((r, o) => {
        e.font = r.font, e.fillStyle = r.color || "#fff", e.fillText(r.text, l, i + (o - 0.5) * 22);
      }), e.restore();
    }
  };
  _initChart() {
  ...
  _updateCenterForSlice(t) {
    ...
    if (!n) return;
    const a = Math.round(e[t] / n * 100), l = this._chart.data.labels[t];
  ...
  _updateCalculator() {
    const i = {
      ...
      6: 0.1,
      12: 0.15
    }, r = (s) => s >= 100 ? 0.1 : s >= 40 ? 0.05 : 0, o = Number(this.$pageUpdates.value) || 0, c = Number(this.$blogs.value) || 0, p = Number(this.$videos.value) || 0, h = Number(this.$backlinks.value) || 0, u = Number(this.$contractLength.value), m = [...this.$pageUpdateMode].find(
      (s) => s.checked
    )?.value;
    let g = 0;
    if (m === "one") {
    ...
    const C = `$${Math.round(g).toLocaleString()}`;
    this.$oneTimeCost.textContent !== C && (this.$oneTimeCost.textContent = C);
    let b = 0;
    if (m === "monthly") {
      let s = o * 425;
      s *= 1 - r(o), b += s;
    ...
    b += c * 375;
    const T = this.$techSeo.value === "yes" ? 600 : 0, E = p * 675 + h * 625;
    let f = 0;
    this.$reporting.value === "monthly" && (f = 600), this.$reporting.value === "quarterly" && (f = 333);
    const d = i[u] || 0, v = Math.ceil(b * (1 - d)), y = Math.ceil(T * (1 - d)), x = Math.ceil(E * (1 - d)), w = f > 0 ? Math.ceil(f * (1 - d)) : 0, k = v + y + x + w;
    this._currentTotal = k;
    const _ = `$${k.toLocaleString()}`;
    this.$monthlyCost.textContent !== _ && (this.$monthlyCost.textContent = _), this.$pagesLabel.textContent = o > 0 ? m === "monthly" ? `${o} pages / month` : `${o} pages (one-time)` : "None", this.$blogsLabel.textContent = c > 0 ? `${c} / month` : "None", this.$videosLabel.textContent = p > 0 ? `${p} / month` : "None", this.$backlinksLabel.textContent = h > 0 ? `${h} links` : "None", this.$techLabel.textContent = this.$techSeo.value === "yes" ? "$600 / month" : "None", this.$reportingLabel.textContent = this.$reporting.value === "one" ? "One-Time ($1,000)" : this.$reporting.value === "monthly" ? "Monthly ($600 / mo)" : this.$reporting.value === "quarterly" ? "Quarterly ($333 / mo)" : "None", this.$termLabel.textContent = u === 1 ? "One Month" : `${u} Months`, this._updateChartData(v, y, x, w), this._emitUpdateEvent({
      totals: {
        monthly: k,
        oneTime: Math.round(g)
      },
      breakdown: { onPage: v, tech: y, offPage: x, reporting: w },
      inputs: {
        pages: o,
        pageMode: m,
        blogs: c,
