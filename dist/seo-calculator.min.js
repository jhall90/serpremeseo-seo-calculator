...
</div>`, L = ":host{display:block;font-family:var(--font-family, system-ui);font-size:16px;--font-xs: .75rem;--font-sm: .875rem;--font-md: 1rem;--font-lg: 1.25rem;--font-xl: 2rem;--ui-option-font-size: .875rem;--brand-top: var(--brand-top, #0e2a59);--brand-bottom: var(--brand-bottom, #163a7b);--chart-onpage: var(--chart-onpage, #00cc66);--chart-tech: var(--chart-tech, #ff6a00);--chart-offpage: var(--chart-offpage, #ffcc00);--chart-reporting: var(--chart-reporting, #7dd3fc);--bg-light: var(--bg-light, #ffffff);--panel-light: var(--panel-light, #f8fafc);--text-light: var(--text-light, #111111);--bg-dark: var(--bg-dark, #0f172a);--panel-dark: var(--panel-dark, #1e293b);--text-dark: var(--text-dark, #f1f5f9)}.calculator{container-type:inline-size;container-name:calculator;display:flex;max-width:1100px;margin:0 auto;padding:2cqw;border-radius:1.25rem;background:var(--panel-light);color:var(--text-light);overflow:hidden}:host(.dark) .calculator{background:var(--panel-dark);color:var(--text-dark)}@container calculator (max-width: 600px){.calculator{flex-direction:column;padding:1.5cqw;--font-xs: .875rem;--font-sm: 1rem;--font-md: 1.125rem;--font-lg: 1.375rem;--font-xl: 2.25rem}}@container calculator (min-width: 601px) and (max-width: 1024px){.calculator{flex-direction:column;--font-xs: .8125rem;--font-sm: .9375rem;--font-md: 1.0625rem;--font-lg: 1.3125rem;--font-xl: 2.125rem}}.left-panel{flex:1;padding-right:2cqw}.left-panel h1{text-align:center}@container calculator (max-width: 1024px){.left-panel{width:100%}}.right-panel{width:22.5rem;padding:2cqw;border-radius:.75rem;color:#fff;background:linear-gradient(180deg,var(--brand-top),var(--brand-bottom))}@container calculator (max-width: 1024px){.right-panel{width:100%;padding:0cqw}}.theme-toggle{margin-bottom:1.25rem;padding:.5rem 1rem;border-radius:.375rem;border:1px solid rgba(0,0,0,.2);background:var(--panel-light);color:var(--text-light);font-size:var(--font-sm);font-weight:500;cursor:pointer;width:fit-content}:host(.dark) .theme-toggle{background:var(--panel-dark);color:var(--text-dark);border-color:#ffffff40}.theme-toggle:focus-visible{outline:2px solid currentColor;outline-offset:2px}.summary{display:flex;flex-direction:column;gap:.75rem;padding:4cqw}.summary h2{margin:.5rem 0 .25rem;font-size:var(--font-lg);font-weight:600;text-align:center}.summary h1{margin:0 0 1rem;font-size:var(--font-xl);font-weight:700;text-align:center}.summary ul{list-style:none;margin-top:.75rem;padding:0}.summary ul li{display:flex;justify-content:space-between;font-size:var(--ui-option-font-size)}.input-row{margin-bottom:1.25rem;position:relative;z-index:1}.input-row:has(.info-icon:hover),.input-row:has(.info-icon:focus-visible){z-index:10}.input-row label{font-size:var(--ui-option-font-size);font-weight:500;line-height:1.6;opacity:.9;display:block}.input-row input,.input-row select{width:100%;height:2.75rem;padding:.5rem .75rem;border-radius:.5rem;box-sizing:border-box;background:var(--panel-light);color:var(--text-light);border:1px solid rgba(0,0,0,.15);font-size:var(--ui-option-font-size);appearance:none}:host(.dark) .input-row input,:host(.dark) .input-row select{background:var(--panel-dark);color:var(--text-dark);border-color:#ffffff40}.input-row select option{background:var(--panel-light);color:var(--text-light)}:host(.dark) .input-row select option{background:var(--panel-dark);color:var(--text-dark)}.label-with-info{display:flex;align-items:center;gap:.375rem}.label-with-info .info-icon{position:relative}.label-with-info .tooltip{position:absolute;top:calc(100% + .5rem);left:50%;transform:translate(-50%);max-width:min(16rem,90%);padding:.75rem 1rem;border-radius:.5rem;background:#0f172a;color:#fff;font-size:.75rem;line-height:1.4;white-space:normal;text-align:left;opacity:0;pointer-events:none;transition:opacity .2s ease;z-index:20}.label-with-info .info-icon:hover .tooltip,.label-with-info .info-icon:focus-visible .tooltip{opacity:1;transform:translateY(0)}.chart-wrap{max-width:18rem;margin:1.25rem auto 0}@container calculator (max-width: 600px){.chart-wrap{max-width:26rem;padding-inline:4cqw}}.chart-wrap canvas{width:100%;aspect-ratio:1/1}.legend{display:grid;grid-template-columns:repeat(auto-fit,minmax(8rem,1fr));gap:.5rem;margin-top:.75rem;justify-items:center}.legend span{display:flex;align-items:center;gap:.375rem}.legend .dot{width:.625rem;height:.625rem;border-radius:50%}.legend .dot.onpage{background:var(--chart-onpage)}.legend .dot.tech{background:var(--chart-tech)}.legend .dot.offpage{background:var(--chart-offpage)}.legend .dot.reporting{background:var(--chart-reporting)}@container calculator (max-width: 600px){.legend{grid-template-columns:repeat(2,1fr)}}@media(max-width:600px){.calculator{flex-direction:column}}";
class E extends HTMLElement {
  ...
  _updateCalculator() {
    const s = {
      ...
      6: 0.1,
      12: 0.15
    }, o = (C) => C >= 100 ? 0.1 : C >= 40 ? 0.05 : 0, i = Number(this.$pageUpdates.value) || 0, h = Number(this.$blogs.value) || 0, d = Number(this.$videos.value) || 0, p = Number(this.$backlinks.value) || 0, g = Number(this.$contractLength.value);
    let m = i * 425;
    m *= 1 - o(i);
    let u = 0;
    this.$reporting.value === "one" && (u = 1e3);
    const w = Math.round(m + u);
    this.$oneTimeCost.textContent = `$${w.toLocaleString()}`;
