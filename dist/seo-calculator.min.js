...
class E extends HTMLElement {
  ...
  _readPageCssVars() {
    ...
    t.forEach((n) => {
      ...
      r && this.style.setProperty(n, r);
    });
  }
  _applyAttributeOverrides() {
    [...this.attributes].forEach((t) => {
      const e = "--" + t.name.replace(/-(.)/g, (n, r) => r.toLowerCase());
  ...
  _centerTextPlugin = {
    id: "centerText",
    afterDraw: (t) => {
      const { ctx: e, chartArea: n } = t, r = t.$centerText;
      if (!r?.length) return;
      const l = n.left + n.width / 2, s = n.top + n.height / 2;
      e.save(), e.textAlign = "center", e.textBaseline = "middle", r.forEach((i, o) => {
        e.font = i.font, e.fillStyle = i.color || "#fff", e.fillText(i.text, l, s + (o - 0.5) * 22);
      }), e.restore();
    }
  };
  _initChart() {
  ...
  _updateCenterForSlice(t) {
    const e = this._chart.data.datasets[0].data, n = e.reduce((s, i) => s + i, 0);
    if (!n) return;
    const r = Math.round(e[t] / n * 100), l = this._chart.data.labels[t];
    this._chart.$centerText = [
      { text: `${r}%`, font: "700 26px sans-serif" },
      {
        text: l,
  ...
  _updateCalculator() {
    const s = {
      ...
      6: 0.1,
      12: 0.15
    }, i = (a) => a >= 100 ? 0.1 : a >= 40 ? 0.05 : 0, o = Number(this.$pageUpdates.value) || 0, d = Number(this.$blogs.value) || 0, h = Number(this.$videos.value) || 0, u = Number(this.$backlinks.value) || 0, g = Number(this.$contractLength.value), m = [...this.$pageUpdateMode].find(
      (a) => a.checked
    )?.value;
    let f = 0;
    if (m === "one") {
    ...
    this.$reporting.value === "one" && (f += 1e3);
    const y = `$${Math.round(f).toLocaleString()}`;
    this.$oneTimeCost.textContent !== y && (this.$oneTimeCost.textContent = y);
    let b = 0;
    if (m === "monthly") {
      let a = o * 425;
      a *= 1 - i(o), b += a;
    ...
    b += d * 375;
    const $ = this.$techSeo.value === "yes" ? 600 : 0, _ = h * 675 + u * 625;
    let c = 0;
    this.$reporting.value === "monthly" && (c = 600), this.$reporting.value === "quarterly" && (c = 333);
    const p = s[g] || 0, x = Math.ceil(b * (1 - p)), w = Math.ceil($ * (1 - p)), k = Math.ceil(_ * (1 - p)), C = c > 0 ? Math.ceil(c * (1 - p)) : 0, v = x + w + k + C;
    this._currentTotal = v, this.$monthlyCost.textContent !== `$${v.toLocaleString()}` && (this.$monthlyCost.textContent = `$${v.toLocaleString()}`), this.$pagesLabel.textContent = o > 0 ? m === "monthly" ? `${o} pages / month` : `${o} pages (one-time)` : "None", this.$blogsLabel.textContent = d > 0 ? `${d} / month` : "None", this.$videosLabel.textContent = h > 0 ? `${h} / month` : "None", this.$backlinksLabel.textContent = u > 0 ? `${u} links` : "None", this.$techLabel.textContent = this.$techSeo.value === "yes" ? "$600 / month" : "None", this.$reportingLabel.textContent = this.$reporting.value === "one" ? "One-Time ($1,000)" : this.$reporting.value === "monthly" ? "Monthly ($600 / mo)" : this.$reporting.value === "quarterly" ? "Quarterly ($333 / mo)" : "None", this.$termLabel.textContent = g === 1 ? "One Month" : `${g} Months`, this._updateChartData(x, w, k, C);
  }
  /* =====================
      CHART DATA UPDATE (RESTORED)
  ===================== */
  _updateChartData(t, e, n, r) {
    if (!this._chart) return;
    const l = [t, e, n, r];
    this._chart.data.datasets[0].data = l, this._chart.update(), this._updateCenterDefault();
  }
}
