/*! seo-calculator-web-component | MIT License | 2025 */
...
<div class="calculator">
    ...
    <div class="right-panel">
        ...
        `}connectedCallback(){this._cacheDom(),this._readPageCssVars(),this._applyAttributeOverrides(),this._initThemeListener(),this._updateThemeClass(),this._updateResponsiveClasses(),window.addEventListener("resize",()=>this._updateResponsiveClasses()),this._loadChartJs().then(()=>{this._initChart(),this._attachEvents(),this._updateCalculator()})}_cacheDom(){const t=this.shadowRoot;this.$businessType=t.getElementById("businessType"),this.$numPages=t.getElementById("numPages"),this.$contentNeed=t.getElementById("contentNeed"),this.$backlinkPack=t.getElementById("backlinkPack"),this.$techComplexity=t.getElementById("techComplexity"),this.$reportLevel=t.getElementById("reportLevel"),this.$engagementTerm=t.getElementById("engagementTerm"),this.$monthlyCost=t.getElementById("monthlyCost"),this.$businessTypeLabel=t.getElementById("businessTypeLabel"),this.$websiteSizeLabel=t.getElementById("websiteSizeLabel"),this.$contentPlanLabel=t.getElementById("contentPlanLabel"),this.$backlinkLabel=t.getElementById("backlinkLabel"),this.$techLabel=t.getElementById("techLabel"),this.$reportingLabel=t.getElementById("reportingLabel"),this.$termLabel=t.getElementById("termLabel"),this.$chartCanvas=t.getElementById("paymentChart"),this.$themeToggle=t.querySelector(".theme-toggle")}_readPageCssVars(){const t=["--brand-top","--brand-bottom","--chart-onpage","--chart-tech","--chart-offpage","--font-family","--bg-light","--panel-light","--text-light","--bg-dark","--panel-dark","--text-dark"],e=getComputedStyle(document.documentElement);t.forEach(i=>{const a=e.getPropertyValue(i).trim();a&&this.style.setProperty(i,a)})}_applyAttributeOverrides(){[...this.attributes].forEach(t=>{const e="--"+t.name.replace(/-(.)/g,(i,a)=>a.toLowerCase());this.style.setProperty(e,t.value)})}attributeChangedCallback(){this._applyAttributeOverrides(),this._updateChartColors()}_initThemeListener(){this.$themeToggle.addEventListener("click",()=>{const t=this.getAttribute("theme")||"system",e=t==="light"?"dark":t==="dark"?"system":"light";this.setAttribute("theme",e),this._themeAttr=e,this._updateThemeClass()})}_updateThemeClass(){const t=this.getAttribute("theme")||"system",e=window.matchMedia("(prefers-color-scheme: dark)").matches,i=document.body.classList.contains("dark-mode");let a=!1;t==="dark"?a=!0:t==="light"?a=!1:a=i||e,a?this.classList.add("dark"):this.classList.remove("dark")}_loadChartJs(){return window.Chart?Promise.resolve():new Promise(t=>{const e=document.createElement("script");e.src="https://cdn.jsdelivr.net/npm/chart.js",e.onload=t,document.head.appendChild(e)})}_initChart(){const t=this.$chartCanvas.getContext("2d");this._chart=new Chart(t,{type:"doughnut",data:{labels:["On-Page","Technical","Off-Page"],datasets:[{data:[0,0,0],backgroundColor:[this._css("--chart-onpage"),this._css("--chart-tech"),this._css("--chart-offpage")],borderWidth:0}]},options:{cutout:"70%",plugins:{legend:!1}}})}_css(t){return getComputedStyle(this).getPropertyValue(t).trim()}_updateChartColors(){this._chart&&(this._chart.data.datasets[0].backgroundColor=[this._css("--chart-onpage"),this._css("--chart-tech"),this._css("--chart-offpage")],this._chart.update())}_attachEvents(){[this.$businessType,this.$numPages,this.$contentNeed,this.$backlinkPack,this.$techComplexity,this.$reportLevel,this.$engagementTerm].forEach(t=>t.addEventListener("input",()=>this._updateCalculator()))}_updateCalculator(){const t={small:{on:25,tech:300,link:150,report:150},mid:{on:45,tech:500,link:250,report:250},enterprise:{on:75,tech:1e3,link:400,report:500}},e={none:0,basic:3,standard:6,aggressive:10},i={none:0,some:.5,ongoing:1},a={low:1,medium:1.5,high:2},s={basic:1,standard:1.5,premium:2},c={1:0,3:.05,6:.1,12:.15},n=this.$businessType.value,o=Math.max(1,Number(this.$numPages.value)||1),l=o*t[n].on,g=l*i[this.$contentNeed.value],r=l+g,h=t[n].tech*a[this.$techComplexity.value],p=t[n].link*e[this.$backlinkPack.value],m=t[n].report*s[this.$reportLevel.value];let d=r+h+p+m;d*=1-c[this.$engagementTerm.value],this.$monthlyCost.textContent="$"+Math.round(d).toLocaleString(),this.$businessTypeLabel.textContent=n,this.$websiteSizeLabel.textContent=o+" pages",this.$contentPlanLabel.textContent=this.$contentNeed.value,this.$backlinkLabel.textContent=this.$backlinkPack.value,this.$techLabel.textContent=this.$techComplexity.value,this.$reportingLabel.textContent=this.$reportLevel.value,this.$termLabel.textContent=this.$engagementTerm.value+" months",this._updateChartData(r,h,p)}_updateChartData(t,e,i){this._chart.data.datasets[0].data=[t,e,i],this._chart.update()}_updateResponsiveClasses(){const t=window.innerWidth,i=window.innerHeight>t,a=t<=550,s=t>550&&t<=1024;if(this.classList.remove("mobile","tablet"),a){this.classList.add("mobile");return}if(s&&i){this.classList.add("tablet");return}}}customElements.define("seo-calculator",SeoCalculator);
