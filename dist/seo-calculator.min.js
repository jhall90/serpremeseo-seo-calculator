...
class E extends HTMLElement {
  ...
  _updateCalculator() {
    ...
    const k = Math.round(m + f);
    this.$oneTimeCost.textContent = `$${k.toLocaleString()}`;
    const C = h * 375, $ = this.$techSeo.value === "yes" ? 600 : 0, _ = d * 675 + p * 625;
    let l = 0;
    this.$reporting.value === "monthly" && (l = 600), this.$reporting.value === "quarterly" && (l = 333);
    const c = s[g] || 0, u = Math.ceil(C * (1 - c)), b = Math.ceil($ * (1 - c)), v = Math.ceil(_ * (1 - c)), x = l > 0 ? Math.ceil(l * (1 - c)) : 0, y = u + b + v + x;
    this._currentTotal = y, this.$monthlyCost.textContent = `$${y.toLocaleString()}`, this.$pagesLabel.textContent = i > 0 ? `${i} pages` : "None", this.$blogsLabel.textContent = h > 0 ? `${h} / month` : "None", this.$videosLabel.textContent = d > 0 ? `${d} / month` : "None", this.$backlinksLabel.textContent = p > 0 ? `${p} links` : "None", this.$techLabel.textContent = this.$techSeo.value === "yes" ? "$600 / month" : "None", this.$reportingLabel.textContent = this.$reporting.value === "one" ? "One-Time ($1,000)" : this.$reporting.value === "monthly" ? "Monthly ($600 / mo)" : this.$reporting.value === "quarterly" ? "Quarterly ($333 / mo)" : "None", this.$termLabel.textContent = g === 1 ? "One Month" : `${g} Months`, this._updateChartData(u, b, v, x);
  }
  /* =====================
